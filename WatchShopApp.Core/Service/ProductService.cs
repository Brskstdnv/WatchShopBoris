...
namespace WatchShopApp.Core.Service
{
    public class ProductService : IProductService
    {
        ...
        public List<Product> GetProducts(string searchStringCategoryName, string searchStringManufacturerName, string searchStringProductName)
        {
            ...
            {
                query = query.Where(p => p.Category.CategoryName.ToLower().Contains(searchStringCategoryName.ToLower()));
            }

            if (!string.IsNullOrEmpty(searchStringManufacturerName))
            {
                query = query.Where(p => p.Manufacturer.Name.ToLower().Contains(searchStringManufacturerName.ToLower()));
            }

            if (!string.IsNullOrEmpty(searchStringProductName))
            {
                query = query.Where(p => p.ProductName.ToLower().Contains(searchStringProductName.ToLower()));
            }
